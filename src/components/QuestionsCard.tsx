...
export function QuestionsCard({ question }: any) {
  ...
  return (
    <div className="p-6 rounded-2xl border border-gray-200 dark:border-gray-800 hover:border-rose-300 dark:hover:border-rose-700 transition-all duration-200 cursor-pointer">
      <Link href={`question/${question.$id}/${slugify(question.title)}`}>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        >
          <Link href={`user/${question.author.$id}/${slugify(question.author.name)}`}>{question.author.name}</Link> •{" "}
...
export function QuestionsCardSkeleton() {
